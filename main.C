	...
	auto f = [=](int iProcess){
		...
		for (int i=0; i<numberEventsToSavePerProcess; i++){
        	...
        	cout << "Pi0 fit range: " << fitRangePi02[0] << ", " << fitRangePi02[1] << endl;
                ...
                string sThread = to_string(iThread);
			...
			for ( int iVar=0; iVar<dim; ++iVar ){
			    ...
			    for ( auto initSigFrac : sigFracs ){
			        ...
			        if(verbose){logFile <<	"\tPrepping 2D fits:	 " << duration2 << "ms" << endl;}
                                ...
                                //}
                                //TThread::Lock();
                                RooFitResult* roo_result = rooSigPlusBkg.fitTo(rooData, Range(("roo_fitRangeMpi0"+sThread+",roo_fitRangeMeta"+sThread).c_str()), Minimizer("Minuit2"), Save(), PrintLevel(-1));
                                //TThread::UnLock();
                                //roo_result->Print("v");
			        ...
			        if(verbose){logFile <<	"\tFitted hist with some initialization: " << duration2 << "ms" << endl;}
        		        ...
        		        if ( selectRandomIdxToSave.find(ientry) != selectRandomIdxToSave.end()) {
			                ...
			                if(verbose){logFile <<	"\tCompleted drawing on pad 1: " << duration2 << "ms" << endl;}
                                        ...
                                        RooAbsPdf* rooGaus2DProjMeta = rooGaus2D.createProjection(roo_Meta);
                                        rooGaus2DProjMeta->setNormRange(("roo_fitRangeMeta"+sThread).c_str());
                                        rooGaus2DProjMeta->plotOn(roo_Mpi0_frame);
                                        RooAbsPdf* rooBkgProjMeta = rooBkg.createProjection(roo_Meta);
                                        rooBkgProjMeta->setNormRange(("roo_fitRangeMeta"+sThread).c_str());
                                        rooBkgProjMeta->plotOn(roo_Mpi0_frame, LineStyle(kDashed),LineColor(kOrange));
