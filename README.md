...
- Can we take this a bit further and talk about representative subsets? Do we need to consider all potential neighbors or can we choose some subset as long as the phase space distribution is the same? A flag is implemented to allow users to choose a subset size. This is important to consider for larger datasets, and if considering bootstrapping of the errors. 
