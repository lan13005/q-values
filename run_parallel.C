...
void run_parallel(int iProcess, int nProcess){
	// Starting timing
	clock_t start;
	double duration;
	start = clock();
	...
	cout << "Actual Total Entries: " << nentries << endl;
	//nentries=1000;
	...
	ofstream entireLogFile;
    	logFile.open(("logEventChiSqQValue_process"+to_string(iProcess)+".txt").c_str());
	...
	// We will use a shifted_ientry to keep track of which entries we will get from the tree. We will simply use ientry when filling the arrays.  
	int shifted_ientry;
	for (int ientry=0; ientry<batchEntries; ientry++)
	{
		shifted_ientry = ientry+lowest_nentry;	
		dataTree->GetEntry(shifted_ientry);
		Metas[ientry]=Meta;
		Mpi0s[ientry]=Mpi0;
	...
	for (int ientry=0; ientry<batchEntries; ientry++){ 
		...
		shifted_ientry = ientry+lowest_nentry;
		cout << "Starting event " << shifted_ientry << "/" << largest_nentry << " ---- Time: " << duration << "s" << endl;
		mapDistToJ.clear();
		distances.clear();
		allCanvases->Clear();
		...
		if ( selectRandomIdxToSave.find(ientry) != selectRandomIdxToSave.end()) {
			logFile << shifted_ientry << "\t" << qvalue << "\t" << fit->GetChisquare()<< "\t" << Mpi0s[ientry] << endl;
			...
			//sigFit->Draw("same");
  			...
  			sigFit->Draw("SAME FC");
			etaLine->Draw("same");
			allCanvases->SaveAs(("Meta-event"+std::to_string(shifted_ientry)+".png").c_str());
		}
	}
